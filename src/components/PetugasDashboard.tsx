...
import { PeminjamanAktifDashboard } from './PeminjamanAktifDashboard';
...
export const PetugasDashboard: React.FC<PetugasDashboardProps> = ({
  ...
  const [petugasMainView, setPetugasMainView] = useState<'kasir' | 'peminjaman_aktif'>('kasir');
  ...
  // Switch to Kasir view with specific Guru from Active Dashboard
  const handleGoToKasirWithGuru = (guru: Guru, targetTx?: Transaksi) => {
    setScannedGuru(guru);
    setPetugasMainView('kasir');
    if (targetTx) {
      setActiveTab('kembali');
      const sisa = targetTx.jumlah - (targetTx.jumlah_kembali || 0);
      setReturnSelection({ [targetTx.id]: sisa });
      showToast(`Pengembalian dibuka untuk ${guru.nama}`, 'info');
    } else {
      setActiveTab('pinjam');
      showToast(`Guru dipilih di kasir: ${guru.nama}`, 'info');
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 w-full text-left overflow-hidden">
      ...
      {/* Sub-Header Navigation Tabs: Kasir Scan vs Dashboard Peminjaman Aktif */}
      <div className="bg-white border-b border-[#074A69]/15 px-4 sm:px-6 py-2.5 flex items-center justify-between gap-3 z-10 shadow-2xs">
        <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-xl border border-slate-200">
          <button
            type="button"
            onClick={() => setPetugasMainView('kasir')}
            className={`px-3.5 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-extrabold flex items-center gap-2 transition cursor-pointer ${
              petugasMainView === 'kasir'
                ? 'bg-[#074A69] text-white shadow-xs'
                : 'text-gray-600 hover:text-gray-900 hover:bg-slate-200/60'
            }`}
          >
            <ScanLine className="w-4 h-4" />
            <span>Kasir / Scan Transaksi</span>
          </button>

          <button
            type="button"
            onClick={() => setPetugasMainView('peminjaman_aktif')}
            className={`px-3.5 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-extrabold flex items-center gap-2 transition cursor-pointer relative ${
              petugasMainView === 'peminjaman_aktif'
                ? 'bg-[#074A69] text-white shadow-xs'
                : 'text-gray-600 hover:text-gray-900 hover:bg-slate-200/60'
            }`}
          >
            <Activity className={`w-4 h-4 ${petugasMainView === 'peminjaman_aktif' ? 'text-emerald-400' : 'text-[#074A69]'}`} />
            <span>Dashboard Peminjaman Aktif</span>
            {activeTransaksis.length > 0 && (
              <span
                className={`px-2 py-0.5 rounded-full text-[10px] font-black transition ${
                  petugasMainView === 'peminjaman_aktif'
                    ? 'bg-emerald-400 text-slate-950'
                    : 'bg-[#074A69] text-white'
                }`}
              >
                {activeTransaksis.length}
              </span>
            )}
          </button>
        </div>
        ...
        <div className="hidden md:flex items-center gap-3 text-xs text-gray-500 font-medium">
          <span className="flex items-center gap-1.5 font-semibold text-gray-700 bg-sky-50 border border-sky-100 px-3 py-1 rounded-full">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
            </span>
            <span>{activeTransaksis.length} Unit / Transaksi Sedang Dipinjam</span>
          </span>
        </div>
      </div>
      ...
      <main className="flex-1 p-4 sm:p-6 overflow-y-auto flex flex-col gap-6 text-left">
        {petugasMainView === 'peminjaman_aktif' ? (
          <PeminjamanAktifDashboard
            db={db}
            setDb={setDb}
            onGoToKasirWithGuru={handleGoToKasirWithGuru}
            showToast={showToast}
          />
        ) : (
          <>
            ...
            {/* ACTIVE LOANS GRID (Quick preview in Kasir tab) */}
              ...
              <div className="flex items-center justify-between mb-4 border-b border-[#074A69]/15 pb-3">
                <div>
                  <h3 className="font-extrabold text-base text-gray-900 flex items-center gap-2">
                    <Clock className="w-5 h-5 text-[#074A69]" /> Ringkasan Peminjaman Berlangsung (Aktif)
                  </h3>
                  <p className="text-xs text-gray-500 mt-0.5">
                    Daftar ringkas perangkat yang sedang dipinjam guru hari ini.
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => setPetugasMainView('peminjaman_aktif')}
                    className="text-xs font-bold text-[#074A69] bg-sky-50 hover:bg-sky-100 px-3 py-1.5 rounded-xl border border-sky-200 transition cursor-pointer flex items-center gap-1.5"
                  >
                    <Activity className="w-3.5 h-3.5 text-[#074A69]" />
                    <span>Buka Dashboard Lengkap</span>
                  </button>
                  <span className="bg-[#074A69] text-white text-xs font-bold px-3 py-1 rounded-full shadow-2xs">
                    {activeTransaksis.length} Aktif
                  </span>
                </div>
              </div>
